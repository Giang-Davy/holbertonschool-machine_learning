...
class Node:
    ...
    def update_indicator(self):
        ...
        def is_large_enough(x):
            if x.ndim == 1:
                return np.array([x[key] >= self.lower[key] for key in self.lower.keys()])
            else:
                return np.all(
        ...
        def is_small_enough(x):
            if x.ndim == 1:
                return np.array([x[key] <= self.upper[key] for key in self.upper.keys()])
            else:
                return np.all(
    ...
    def pred(self, x):
        """
        Effectue une prédiction pour un seul exemple x en fonction
        de la caractéristique et du seuil de ce nœud.
        """
        if self.is_leaf:
            return self.value  # Retourne la valeur de la feuille
        if x[self.feature] <= self.threshold:
            if self.left_child is None:
                return self.value  # Si c'est une feuille
...
class Decision_Tree():
    ...
    # Mettre à jour l'indicateur de la racine avant de continuer
    ...
    # Vérification si l'indicateur est défini après mise à jour
        if self.root.indicator is None:
            raise ValueError("L'indicateur du nœud racine n'a pas été défini correctement.")
    
    # Définir la fonction de prédiction
        self.predict = lambda A: np.array([self.root.indicator(A[i]) for i in range(A.shape[0])])
