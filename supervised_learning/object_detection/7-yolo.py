...
class Yolo:
    ...
    def predict(self, folder_path):
        results = []
        imgs, img_paths = self.load_images(folder_path)

        if not imgs:
            return [], []

        processed_imgs, img_shapes = self.preprocess_images(imgs)
        model_outputs = self.model.predict(processed_imgs)
        ...
        for i, img in enumerate(imgs):
            ...
            boxes, box_confidence, box_probs = self.process_outputs(output_layers, img_shapes[i])
            filtered_boxes, box_classes, box_scores = self.filter_boxes(boxes, box_confidence, box_probs)
            final_boxes, final_classes, final_scores = self.non_max_suppression(filtered_boxes, box_classes, box_scores)

            base_name = os.path.basename(img_paths[i])
            self.show_boxes(img.copy(), final_boxes, final_classes, final_scores, base_name)
            results.append((final_boxes, final_classes, final_scores))
